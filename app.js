...
function saveMarket(){var name=document.getElementById('m-name').value.trim();if(!name){alert('Market name is required.');return;}var stallTypesVal=state._tempStallTypes.filter(t=>t.name&&t.name.trim()).map(t=>({id:t.id||('st-'+Date.now()),name:t.name.trim(),fee:t.fee||FEE,color:t.color||'#6b7280'}));if(!stallTypesVal.length)stallTypesVal=defaultStallTypes(FEE);var m={id:state.editMarketId||uid(),name,desc:document.getElementById('m-desc').value.trim(),header:document.getElementById('m-header').value.trim(),fee:stallTypesVal[0].fee,stallTypes:stallTypesVal,capacity:parseInt(document.getElementById('m-capacity').value)||30,dates:[...state._tempDates],banner:state._tempBanner||null,published:false,notes:''};if(state.editMarketId){var i=state.markets.findIndex(x=>x.id===state.editMarketId);m.published=state.markets[i].published;m.notes=state.markets[i].notes||'';state.markets[i]=m;}else state.markets.push(m);sbSave('markets',marketToDb(m));showPage('markets');renderMarkets();updateMetrics();}
