...
var state={vendors:[],markets:[],editMarketId:null,notesMarketId:null,approvedToday:0,expandedRows:{},filterPayment:'',hideHints:false,_approveQueue:[],_removeQueue:[],_tempDates:[],_tempBanner:null,_tempStallTypes:[],_menuVendorId:null,_payMarketId:null};
var FEE=350;
...
function openMarketModal(id){state.editMarketId=id;var m=id?state.markets.find(x=>x.id===id):null;document.getElementById('market-modal-title').textContent=m?'Edit market':'New market';document.getElementById('m-name').value=m?m.name:'';document.getElementById('m-desc').value=m?(m.desc||''):'';document.getElementById('m-header').value=m?(m.header||''):'';document.getElementById('m-fee').value=m?(m.fee||FEE):FEE;state._tempStallTypes=m&&m.stallTypes&&m.stallTypes.length?m.stallTypes.map(t=>({...t})):defaultStallTypes(m?m.fee:FEE);document.getElementById('m-capacity').value=m?(m.capacity||30):30;state._tempDates=m?[...m.dates]:[];state._tempBanner=m?(m.banner||null):null;var prev=document.getElementById('banner-preview'),prompt=document.getElementById('banner-upload-prompt'),rb=document.getElementById('banner-remove-btn');if(state._tempBanner){prev.src=state._tempBanner;prev.style.display='block';prompt.style.display='none';rb.style.display='inline-flex';}else{prev.src='';prev.style.display='none';prompt.style.display='block';rb.style.display='none';}renderDateList();renderStallTypeList();document.getElementById('market-modal').classList.add('open');}
...
function saveMarket(){var name=document.getElementById('m-name').value.trim();if(!name){alert('Market name is required.');return;}var baseFee=parseInt(document.getElementById('m-fee').value)||FEE;var stallTypesVal=state._tempStallTypes.filter(t=>t.name&&t.name.trim()).map(t=>({id:t.id||('st-'+Date.now()),name:t.name.trim(),fee:t.fee||baseFee,color:t.color||'#6b7280'}));if(!stallTypesVal.length)stallTypesVal=defaultStallTypes(baseFee);var m={id:state.editMarketId||uid(),name,desc:document.getElementById('m-desc').value.trim(),header:document.getElementById('m-header').value.trim(),fee:baseFee,stallTypes:stallTypesVal,capacity:parseInt(document.getElementById('m-capacity').value)||30,dates:[...state._tempDates],banner:state._tempBanner||null,published:false,notes:''};if(state.editMarketId){var i=state.markets.findIndex(x=>x.id===state.editMarketId);m.published=state.markets[i].published;m.notes=state.markets[i].notes||'';state.markets[i]=m;}else state.markets.push(m);sbSave('markets',marketToDb(m));closeModal('market-modal');renderMarkets();updateMetrics();}
