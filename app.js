...
function switchAccountType(t){_accountType=t;var ob=document.getElementById('at-organiser');var vb=document.getElementById('at-vendor');var desc=document.getElementById('auth-type-desc');if(ob){ob.style.background=t==='organiser'?'var(--blue)':'none';ob.style.color=t==='organiser'?'#fff':'var(--text2)';}if(vb){vb.style.background=t==='vendor'?'var(--blue)':'none';vb.style.color=t==='vendor'?'#fff':'var(--text2)';}if(desc)desc.textContent=t==='organiser'?'Run and manage your markets, vendors, and applications':'Apply to markets and save your vendor details';var of=document.getElementById('reg-organiser-fields');var vf=document.getElementById('reg-vendor-fields');if(of)of.style.display=t==='organiser'?'block':'none';if(vf)vf.style.display=t==='vendor'?'block':'none';clearAuthMsg();}
...
async function loadUserData(){var[{data:mkts},{data:vnds}]=await Promise.all([_sb.from('markets').select('*').eq('user_id',currentUser.id).order('created_at'),_sb.from('vendors').select('*').eq('user_id',currentUser.id).order('created_at')]);state.markets=(mkts||[]).map(dbToMarket);state.vendors=(vnds||[]).map(dbToVendor);var emails=[...new Set(state.vendors.map(v=>v.email).filter(Boolean))];if(emails.length){var{data:vps}=await _sb.from('vendor_profiles').select('email').in('email',emails);state.verifiedEmails=new Set((vps||[]).map(vp=>(vp.email||'').toLowerCase()));}else{state.verifiedEmails=new Set();}}
