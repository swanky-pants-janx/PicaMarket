...
function renderHistory(tab){if(tab)_historyTab=tab;var tvEl=document.getElementById('hist-tab-vendors'),tmEl=document.getElementById('hist-tab-markets');if(tvEl)tvEl.classList.toggle('active',_historyTab==='vendors');if(tmEl)tmEl.classList.toggle('active',_historyTab==='markets');var vs=document.getElementById('hist-vendors-section'),ms=document.getElementById('hist-markets-section');if(vs)vs.style.display=_historyTab==='vendors'?'':'none';if(ms)ms.style.display=_historyTab==='markets'?'':'none';if(_historyTab==='vendors')renderVendorHistory();else renderMarketHistory();}
...
async function archiveMarket(id){var m=state.markets.find(function(x){return x.id===id;});if(!m)return;if(!confirm('Archive "'+esc(m.name)+'"?\n\nThis will snapshot all vendor and payment data, then remove the market. The archive cannot be undone from this dialog — use "Purge All History" on the History page to delete archived data.'))return;var approvedVendors=state.vendors.filter(function(v){return v.status==='approved'&&v.markets.includes(id);});var snapshot={fee:m.fee,stall_types:m.stallTypes||[],capacity:m.capacity||0,notes:m.notes||'',vendors:approvedVendors.map(function(v){return{name:v.name,email:(v.email||'').toLowerCase(),stall_type:stallTypeLabel(v,m),fee:getStallFee(v,m),payment_status:(v.marketPayments||{})[id]||'outstanding',payment_method:(v.marketMethods||{})[id]||null,approved_at:v.approvedAt||null};})};var{data:archived,error:archErr}=await _sb.from('archived_markets').insert({user_id:currentUser.id,market_name:m.name,market_dates:m.dates||[],data:snapshot}).select().single();if(archErr){alert('Failed to archive market. Please try again.');return;}for(var i=0;i<approvedVendors.length;i++){var v=approvedVendors[i];var newMarkets=v.markets.filter(function(mid){return mid!==id;});var newPayments=Object.assign({},v.marketPayments||{});delete newPayments[id];var newMethods=Object.assign({},v.marketMethods||{});delete newMethods[id];var newStallTypes=Object.assign({},v.marketStallTypes||{});delete newStallTypes[id];var newAttendance=Object.assign({},v.marketAttendance||{});delete newAttendance[id];if(newMarkets.length===0){await sbDel('vendors',v.id);state.vendors=state.vendors.filter(function(x){return x.id!==v.id;});}else{var updated=Object.assign({},v,{markets:newMarkets,marketPayments:newPayments,marketMethods:newMethods,marketStallTypes:newStallTypes,marketAttendance:newAttendance});await sbSave('vendors',vendorToDb(updated));var idx=state.vendors.findIndex(function(x){return x.id===v.id;});if(idx!==-1)state.vendors[idx]=updated;}}await sbDel('markets',id);state.markets=state.markets.filter(function(x){return x.id!==id;});if(archived)state.archivedMarkets.unshift(archived);renderMarkets();renderApproved();updateMetrics();showToast('Market archived.');}
