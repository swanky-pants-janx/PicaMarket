...
function openSettingsMenu(e){e.stopPropagation();var overlay=document.getElementById('settings-overlay');if(overlay.classList.contains('open')){overlay.classList.remove('open');return;}syncSettingsMenu();overlay.classList.add('open');}
...
var FB_TYPE_LABELS={text:'Short text',textarea:'Long text',radio:'Single choice',checkbox:'Multi-choice',select:'Dropdown'};
function renderFormBuilder(){
  var fields=currentUser.customFields||[];
  var container=document.getElementById('fb-fields-container');
  if(!fields.length){container.innerHTML='<div class="card" style="text-align:center;padding:3rem 1rem;color:var(--text3)"><div style="font-size:2.5rem;margin-bottom:0.75rem">📋</div><div style="font-size:14px;font-weight:500;margin-bottom:4px">No custom fields yet</div><div style="font-size:13px">Click <strong>+ Add field</strong> to add questions to your vendor application form.</div></div>';return;}
  container.innerHTML='<div class="card" style="padding:0;overflow:hidden">'+fields.map(function(f,i){var opts=f.options&&f.options.length?'<div style="font-size:11px;color:var(--text3);margin-top:3px">'+f.options.map(esc).join(' · ')+'</div>':'';return'<div class="fb-field-card">'+'<div class="fb-field-left"><span class="fb-type-badge">'+esc(FB_TYPE_LABELS[f.type]||f.type)+'</span><div><div class="fb-field-label">'+esc(f.label)+'</div>'+opts+'</div>'+(f.required?'<span class="badge" style="background:var(--red-bg);color:var(--red);font-size:10px;flex-shrink:0">Required</span>':'')+'</div>'+'<div class="fb-field-actions"><button class="btn small" onclick="moveField(\''+f.id+'\',-1)" '+(i===0?'disabled':'')+'  title="Move up">↑</button><button class="btn small" onclick="moveField(\''+f.id+'\',1)" '+(i===fields.length-1?'disabled':'')+' title="Move down">↓</button><button class="btn small" onclick="openEditField(\''+f.id+'\')">Edit</button><button class="btn small" style="color:var(--red)" onclick="deleteField(\''+f.id+'\')">Delete</button></div>'+'</div>';}).join('')+'</div>';
}
...
function openEditField(id){var f=(currentUser.customFields||[]).find(x=>x.id===id);if(!f)return;_editingFieldId=id;_editingOptions=(f.options||[]).slice();document.getElementById('field-modal-title').textContent='Edit field';document.getElementById('field-type').value=f.type;document.getElementById('field-label').value=f.label;document.getElementById('field-required').checked=!!f.required;syncFieldModal();document.getElementById('field-modal').classList.add('open');}
...
function saveField(){var type=document.getElementById('field-type').value;var label=document.getElementById('field-label').value.trim();var required=document.getElementById('field-required').checked;var hasOpts=['radio','checkbox','select'].includes(type);if(!label){showToast('Enter a question or label');return;}if(hasOpts&&!_editingOptions.length){showToast('Add at least one option');return;}var fields=currentUser.customFields||[];var fieldObj={id:_editingFieldId||uid(),type,label,required};if(hasOpts)fieldObj.options=_editingOptions.slice();if(_editingFieldId){var idx=fields.findIndex(x=>x.id===_editingFieldId);if(idx!==-1)fields[idx]=fieldObj;}else{fields.push(fieldObj);}currentUser.customFields=fields;saveUserSettings();closeModal('field-modal');renderFormBuilder();_updateDashboardCustomFieldsPreview();showToast(_editingFieldId?'Field updated':'Field added');}
function deleteField(id){if(!confirm('Delete this field? Existing vendor responses for this field will no longer be shown.'))return;currentUser.customFields=(currentUser.customFields||[]).filter(x=>x.id!==id);saveUserSettings();renderFormBuilder();_updateDashboardCustomFieldsPreview();showToast('Field deleted');}
function moveField(id,dir){var fields=currentUser.customFields||[];var idx=fields.findIndex(x=>x.id===id);var newIdx=idx+dir;if(newIdx<0||newIdx>=fields.length)return;var tmp=fields[idx];fields[idx]=fields[newIdx];fields[newIdx]=tmp;currentUser.customFields=fields;saveUserSettings();renderFormBuilder();}
function _updateDashboardCustomFieldsPreview(){var container=document.getElementById('dashboard-custom-fields');if(!container)return;renderCustomFieldsInto(container,currentUser.customFields||[]);}
function renderCustomFieldsInto(container,fields){if(!fields||!fields.length){container.innerHTML='';return;}container.innerHTML=fields.map(function(f){var reqMark=f.required?' *':'';var input='';if(f.type==='text')input='<input class="form-input" disabled placeholder="Vendor\'s answer">';else if(f.type==='textarea')input='<textarea class="form-input" disabled rows="3" placeholder="Vendor\'s answer"></textarea>';else if(f.type==='select')input='<select class="form-input" disabled><option value="">Select...</option>'+(f.options||[]).map(o=>'<option>'+esc(o)+'</option>').join('')+'</select>';else if(f.type==='radio')input=(f.options||[]).map(o=>'<label style="display:flex;align-items:center;gap:8px;margin:4px 0"><input type="radio" disabled> '+esc(o)+'</label>').join('');else if(f.type==='checkbox')input=(f.options||[]).map(o=>'<label style="display:flex;align-items:center;gap:8px;margin:4px 0"><input type="checkbox" disabled> '+esc(o)+'</label>').join('');return'<div class="form-group"><label class="form-label">'+esc(f.label)+reqMark+'</label>'+input+'</div>';}).join('');}
...
async function loginAs(user,profile){var saved=profile.settings||{};currentUser={...user,market:profile.market_name,name:profile.coordinator_name,slug:profile.slug,payfastMerchantId:profile.payfast_merchant_id||null,payfastMerchantKey:profile.payfast_merchant_key||null,bankHolder:profile.bank_holder||null,bankName:profile.bank_name||null,bankAccNum:profile.bank_acc_num||null,bankBranch:profile.bank_branch||null,bankAccType:profile.bank_acc_type||null,emailApprovalIntro:saved.approval_email_intro||null,emailReminderIntro:saved.reminder_email_intro||null,blockedEmails:saved.blocked_emails||[],customFields:saved.custom_fields||[]};state.vendors=[];state.markets=[];state.expandedRows={};state.filterPayment='';darkMode=!!saved.dark_mode;state.hideHints=!!saved.hide_hints;document.body.classList.toggle('dark',darkMode);document.getElementById('auth-screen').style.display='none';document.getElementById('dashboard').style.display='block';document.getElementById('nav-brand').innerHTML='<span>'+esc(profile.market_name)+'</span> &ndash; Dashboard';document.getElementById('nav-user').textContent=profile.coordinator_name;document.getElementById('sidebar-user').textContent=profile.coordinator_name;document.getElementById('pub-title').textContent=profile.market_name;await loadUserData();renderPending();updateMetrics();if(state.hideHints)document.querySelectorAll('.page-banner').forEach(el=>el.style.display='none');}
...
// ── DB HELPERS ────────────────────────────────────────────────────
function dbToMarket(r){return{id:r.id,name:r.name,desc:r.description||'',header:r.header||'',fee:r.fee||FEE,stallTypes:r.stall_types||[],capacity:r.capacity||30,dates:r.dates||[],banner:r.banner||null,published:r.published||false,notes:r.notes||''};}
function marketToDb(m){return{id:m.id,user_id:currentUser.id,name:m.name,description:m.desc||'',header:m.header||'',fee:m.fee||FEE,stall_types:m.stallTypes||[],capacity:m.capacity||30,dates:m.dates||[],banner:m.banner||null,published:!!m.published,notes:m.notes||''};}
function dbToVendor(r){return{id:r.id,name:r.name,desc:r.description||'',email:r.email,status:r.status||'pending',markets:r.markets||[],marketPayments:r.market_payments||{},marketMethods:r.market_methods||{},marketStallTypes:r.market_stall_types||{},payStatus:r.pay_status||'outstanding',payMethod:r.pay_method||null,images:r.images||[],submitted:r.submitted_at||'',approvedAt:r.approved_at||'',customResponses:r.custom_responses||{}};}
function vendorToDb(v){var obj={id:v.id,user_id:currentUser.id,name:v.name,description:v.desc||'',email:v.email,status:v.status,markets:v.markets||[],market_payments:v.marketPayments||{},market_methods:v.marketMethods||{},market_stall_types:v.marketStallTypes||{},pay_status:v.payStatus||'outstanding',pay_method:v.payMethod||null,images:v.images||[],submitted_at:v.submitted||'',approved_at:v.approvedAt||''};if(v.customResponses&&Object.keys(v.customResponses).length)obj.custom_responses=v.customResponses;return obj;}
...
// ── NAVIGATION ────────────────────────────────────────────────────
function showPage(p){document.querySelectorAll('.page').forEach(el=>el.classList.remove('active'));document.querySelectorAll('.nav-tab').forEach(el=>el.classList.remove('active'));document.querySelectorAll('.bottom-nav-item').forEach(el=>el.classList.remove('active'));document.querySelectorAll('.sidebar-item').forEach(el=>el.classList.remove('active'));document.getElementById('page-'+p).classList.add('active');var map={approval:0,approved:1,markets:2,'form-builder':3};if(map[p]!==undefined){var nt=document.querySelectorAll('.nav-tab')[map[p]];if(nt)nt.classList.add('active');var bn=document.querySelectorAll('.bottom-nav-item')[map[p]];if(bn)bn.classList.add('active');var si=document.querySelectorAll('.sidebar-item')[map[p]];if(si)si.classList.add('active');}if(p==='approval')renderPending();if(p==='approved')renderApproved();if(p==='markets')renderMarkets();if(p==='form-builder')renderFormBuilder();if(p==='public')renderPublic();if(state.hideHints)document.querySelectorAll('.page-banner').forEach(el=>el.style.display='none');updateMetrics();window.scrollTo(0,0);}
...
function duplicateMarket(id){var m=state.markets.find(x=>x.id===id);if(!m)return;var copy=Object.assign({},m,{id:uid(),name:m.name+' (copy)',published:false,notes:'',stallTypes:(m.stallTypes||[]).map(t=>({...t}))});state.markets.push(copy);sbSave('markets',marketToDb(copy));renderMarkets();updateMetrics();}
...
function openMarketModal(id){state.editMarketId=id;var m=id?state.markets.find(x=>x.id===id):null;document.getElementById('market-modal-title').textContent=m?'Edit market':'New market';document.getElementById('m-name').value=m?m.name:'';document.getElementById('m-desc').value=m?(m.desc||''):'';document.getElementById('m-header').value=m?(m.header||''):'';state._tempStallTypes=m&&m.stallTypes&&m.stallTypes.length?m.stallTypes.map(t=>({...t})):defaultStallTypes(m?m.fee:FEE);document.getElementById('m-capacity').value=m?(m.capacity||30):30;state._tempDates=m?[...m.dates]:[];state._tempBanner=m?(m.banner||null):null;var prev=document.getElementById('banner-preview'),prompt=document.getElementById('banner-upload-prompt'),rb=document.getElementById('banner-remove-btn');if(state._tempBanner){prev.src=state._tempBanner;prev.style.display='block';prompt.style.display='none';rb.style.display='inline-flex';}else{prev.src='';prev.style.display='none';prompt.style.display='block';rb.style.display='none';}renderDateList();renderStallTypeList();document.getElementById('market-modal').classList.add('open');}
...
function saveMarket(){var name=document.getElementById('m-name').value.trim();if(!name){alert('Market name is required.');return;}var stallTypesVal=state._tempStallTypes.filter(t=>t.name&&t.name.trim()).map(t=>({id:t.id||('st-'+Date.now()),name:t.name.trim(),fee:t.fee||FEE,color:t.color||'#6b7280'}));if(!stallTypesVal.length)stallTypesVal=defaultStallTypes(FEE);var m={id:state.editMarketId||uid(),name,desc:document.getElementById('m-desc').value.trim(),header:document.getElementById('m-header').value.trim(),fee:stallTypesVal[0].fee,stallTypes:stallTypesVal,capacity:parseInt(document.getElementById('m-capacity').value)||30,dates:[...state._tempDates],banner:state._tempBanner||null,published:false,notes:''};if(state.editMarketId){var i=state.markets.findIndex(x=>x.id===state.editMarketId);m.published=state.markets[i].published;m.notes=state.markets[i].notes||'';state.markets[i]=m;}else state.markets.push(m);sbSave('markets',marketToDb(m));closeModal('market-modal');renderMarkets();updateMetrics();}

// ── PUBLIC / MARKET INDEX ─────────────────────────────────────────
function renderPublic(){renderPubMarkets();renderVendorFormMarkets();_updateDashboardCustomFieldsPreview();}
...
function submitVendorForm(){
  ...
  var nv={id:uid(),name,desc,email,markets:markets.slice(),images:[..._vendorImages],status:'pending',payStatus:'outstanding',payMethod:null,marketPayments:{},submitted:new Date().toLocaleDateString('en-ZA')};
