...
// ── DB HELPERS ────────────────────────────────────────────────────
function dbToMarket(r){return{id:r.id,name:r.name,desc:r.description||'',header:r.header||'',fee:r.fee||FEE,feeLarge:r.fee_large||null,feeFood:r.fee_food||null,capacity:r.capacity||30,dates:r.dates||[],banner:r.banner||null,published:r.published||false,notes:r.notes||''};}
function marketToDb(m){return{id:m.id,user_id:currentUser.id,name:m.name,description:m.desc||'',header:m.header||'',fee:m.fee||FEE,fee_large:m.feeLarge||null,fee_food:m.feeFood||null,capacity:m.capacity||30,dates:m.dates||[],banner:m.banner||null,published:!!m.published,notes:m.notes||''};}
function dbToVendor(r){return{id:r.id,name:r.name,desc:r.description||'',email:r.email,status:r.status||'pending',markets:r.markets||[],marketPayments:r.market_payments||{},marketMethods:r.market_methods||{},marketStallTypes:r.market_stall_types||{},payStatus:r.pay_status||'outstanding',payMethod:r.pay_method||null,images:r.images||[],submitted:r.submitted_at||'',approvedAt:r.approved_at||''};}
function vendorToDb(v){return{id:v.id,user_id:currentUser.id,name:v.name,description:v.desc||'',email:v.email,status:v.status,markets:v.markets||[],market_payments:v.marketPayments||{},market_methods:v.marketMethods||{},market_stall_types:v.marketStallTypes||{},pay_status:v.payStatus||'outstanding',pay_method:v.payMethod||null,images:v.images||[],submitted_at:v.submitted||'',approved_at:v.approvedAt||''};}
function getStallFee(v,m){if(!m)return FEE;var t=(v&&v.marketStallTypes&&v.marketStallTypes[m.id])||'normal';if(t==='large'&&m.feeLarge)return m.feeLarge;if(t==='food'&&m.feeFood)return m.feeFood;return m.fee||FEE;}
function stallTypeLabel(t){return t==='large'?'Large stall':t==='food'?'Food vendor':'Normal stall';}
function stallTypeBadge(t){if(t==='large')return'<span class="badge" style="background:#ede9fe;color:#7c3aed">Large stall</span>';if(t==='food')return'<span class="badge" style="background:#fff7ed;color:#ea580c">Food vendor</span>';return'';}
...
function renderPending(filter){
  ...
  tb.innerHTML=list.map(v=>{
    ...
    if(exp){var stallInfo=v.markets.map(mid=>{var m=state.markets.find(x=>x.id===mid);var t=(v.marketStallTypes||{})[mid]||'normal';return m?'<div style="display:flex;justify-content:space-between;font-size:12px;padding:3px 0"><span>'+esc(m.name)+' <span style="color:var(--text3)">('+stallTypeLabel(t)+')</span></span><span style="font-weight:500">R'+getStallFee(v,m)+'</span></div>':''}).join('');rows+='<tr class="expand-row"><td colspan="6"><div class="expand-inner"><div><div class="field-label">Email</div><div class="field-value">'+esc(v.email)+'</div></div><div><div class="field-label">Est. fee</div><div class="field-value" style="font-weight:500">R'+(v.markets.reduce((t,mid)=>{var m=state.markets.find(x=>x.id===mid);return t+getStallFee(v,m);},0)).toLocaleString()+'</div></div><div style="grid-column:1/-1"><div class="field-label">Description</div><div class="field-value">'+esc(v.desc)+'</div></div><div style="grid-column:1/-1"><div class="field-label">Markets &amp; stall types</div><div style="margin-top:6px">'+stallInfo+'</div></div>'+photos+'</div></td></tr>';}
    return rows;
  }).join('');
  cards.innerHTML=list.map(v=>{
    ...
    var expand=exp?'<div class="vendor-card-expand"><div class="field-label">Email</div><div class="field-value">'+esc(v.email)+'</div><div class="field-label">Description</div><div class="field-value">'+esc(v.desc)+'</div><div class="field-label">Markets &amp; stall types</div>'+v.markets.map(mid=>{var m=state.markets.find(x=>x.id===mid);var t=(v.marketStallTypes||{})[mid]||'normal';return m?'<div style="display:flex;justify-content:space-between;font-size:12px;padding:3px 0"><span>'+esc(m.name)+' <span style="color:var(--text3)">('+stallTypeLabel(t)+')</span></span><span style="font-weight:500">R'+getStallFee(v,m)+'</span></div>':''}).join('')+'<div class="field-label" style="margin-top:6px">Est. fee</div><div class="field-value" style="font-weight:500">R'+(v.markets.reduce((t,mid)=>{var m=state.markets.find(x=>x.id===mid);return t+getStallFee(v,m);},0)).toLocaleString()+'</div>'+photos+'</div>':'';
...
function mergeApproveVendor(v,sel){var mnames=sel.map(mid=>{var m=state.markets.find(x=>x.id===mid);return m?m.name:mid;}).join(', ');var existing=state.vendors.find(x=>x.id!==v.id&&x.status==='approved'&&x.email===v.email);if(existing){sel.forEach(mid=>{if(!existing.markets.includes(mid))existing.markets.push(mid);if(!existing.marketPayments)existing.marketPayments={};if(!existing.marketPayments[mid])existing.marketPayments[mid]='outstanding';});sbSave('vendors',vendorToDb(existing));sbDel('vendors',v.id);state.vendors=state.vendors.filter(x=>x.id!==v.id);}else{v.status='approved';v.markets=sel.slice();v.payStatus='outstanding';v.payMethod=null;v.marketPayments={};sel.forEach(mid=>{v.marketPayments[mid]='outstanding';});v.approvedAt=new Date().toLocaleDateString('en-ZA');sbSave('vendors',vendorToDb(v));}var paySection='';
  if(currentUser.payfastMerchantId&&currentUser.payfastMerchantKey){
    var links=sel.map(mid=>{var m=state.markets.find(x=>x.id===mid);if(!m)return'';var url=pfUrl(v,m);var t=(v.marketStallTypes||{})[mid]||'normal';return url?'<p style="margin:8px 0"><strong>'+esc(m.name)+'</strong> ('+stallTypeLabel(t)+') — R'+getStallFee(v,m)+': <a href="'+url+'" style="color:#2563eb">Pay now</a></p>':'';}).join('');
  ...
  sendEmail(v.email,'You\'ve been approved — '+esc(currentUser.market),'<p>Hi '+esc(v.name)+',</p><p>Great news! Your vendor application has been approved for the following market'+(sel.length!==1?'s':'')+':</p><p><strong>'+esc(mnames)+'</strong></p>'+paySection+'<p>Thanks,<br>'+esc(currentUser.market)+'</p>');}
...
function renderApproved(filter,payFilter){
  ...
  tb.innerHTML=list.map(v=>{
    ...
    var chips=v.markets.map(mid=>{var m=state.markets.find(x=>x.id===mid);var t=(v.marketStallTypes||{})[mid]||'normal';return m?'<span class="chip">'+esc(m.name)+(t!=='normal'?' ('+stallTypeLabel(t)+')':'')+'</span>':''}).join('');
    ...
    if(exp){
      rows+=v.markets.map(mid=>{
        ...
        var t=(v.marketStallTypes||{})[mid]||'normal';
        ...
        return'<tr style="background:var(--bg2)"><td colspan="2" style="padding:8px 12px 8px 28px;font-size:12px;color:var(--text2)">&#8627; '+(m?esc(m.name):mid)+' <span style="color:var(--text3)">('+stallTypeLabel(t)+')</span></td><td style="padding:8px 12px;font-size:12px;font-weight:500">R'+getStallFee(v,m)+'</td><td style="padding:8px 12px">'+mpb+'</td><td style="padding:8px 12px">'+mmb+'</td><td></td></tr>';
      }).join('');
    }
    return rows;
  }).join('');
...
function openMarketDashboard(mid){
  ...
  if(approvedVendors.length){
    html+='<table class="vendor-table desktop-table"><thead><tr><th>Stall name</th><th>Email</th><th>Stall type</th><th>Fee</th><th>Payment</th><th></th></tr></thead><tbody>'+approvedVendors.map(v=>{var t=(v.marketStallTypes||{})[mid]||'normal';var mp=(v.marketPayments||{})[mid];var pb=mp==='paid'?'<span class="badge paid">Paid</span>':'<span class="badge outstanding">Outstanding</span>';return'<tr><td><strong>'+esc(v.name)+'</strong></td><td style="color:var(--text2)">'+esc(v.email)+'</td><td>'+stallTypeBadge(t)+(t==='normal'?'<span style="font-size:12px;color:var(--text2)">Normal</span>':'')+'</td><td>R'+getStallFee(v,m)+'</td><td>'+pb+'</td><td><button onclick="removeFromMarket(\''+v.id+'\',\''+mid+'\')" style="background:none;border:0.5px solid var(--red);border-radius:6px;cursor:pointer;padding:4px 8px;font-size:12px;color:var(--red)">&#128465; Remove</button></td></tr>';}).join('')+'</tbody></table>'
      +'<div class="mobile-cards">'+approvedVendors.map(v=>{var t=(v.marketStallTypes||{})[mid]||'normal';var mp=(v.marketPayments||{})[mid];var pb=mp==='paid'?'<span class="badge paid">Paid</span>':'<span class="badge outstanding">Outstanding</span>';return'<div class="vendor-card"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px"><strong>'+esc(v.name)+'</strong>'+pb+'</div><div style="font-size:11px;color:var(--text2);margin-bottom:4px">'+esc(v.email)+'</div><div style="font-size:11px;color:var(--text2);margin-bottom:8px">'+stallTypeLabel(t)+' — R'+getStallFee(v,m)+'</div><button onclick="removeFromMarket(\''+v.id+'\',\''+mid+'\')" style="background:none;border:0.5px solid var(--red);border-radius:6px;cursor:pointer;padding:5px 10px;font-size:12px;color:var(--red)">&#128465; Remove</button></div>';}).join('')+'</div>';
...
function openMarketModal(id){state.editMarketId=id;var m=id?state.markets.find(x=>x.id===id):null;document.getElementById('market-modal-title').textContent=m?'Edit market':'New market';document.getElementById('m-name').value=m?m.name:'';document.getElementById('m-desc').value=m?(m.desc||''):'';document.getElementById('m-header').value=m?(m.header||''):'';document.getElementById('m-fee').value=m?(m.fee||FEE):FEE;document.getElementById('m-fee-large').value=m?(m.feeLarge||''):'';document.getElementById('m-fee-food').value=m?(m.feeFood||''):'';document.getElementById('m-capacity').value=m?(m.capacity||30):30;state._tempDates=m?[...m.dates]:[];state._tempBanner=m?(m.banner||null):null;var prev=document.getElementById('banner-preview'),prompt=document.getElementById('banner-upload-prompt'),rb=document.getElementById('banner-remove-btn');if(state._tempBanner){prev.src=state._tempBanner;prev.style.display='block';prompt.style.display='none';rb.style.display='inline-flex';}else{prev.src='';prev.style.display='none';prompt.style.display='block';rb.style.display='none';}renderDateList();document.getElementById('market-modal').classList.add('open');}
...
function saveMarket(){var name=document.getElementById('m-name').value.trim();if(!name){alert('Market name is required.');return;}var feeLargeVal=parseInt(document.getElementById('m-fee-large').value)||null;var feeFoodVal=parseInt(document.getElementById('m-fee-food').value)||null;var m={id:state.editMarketId||uid(),name,desc:document.getElementById('m-desc').value.trim(),header:document.getElementById('m-header').value.trim(),fee:parseInt(document.getElementById('m-fee').value)||FEE,feeLarge:feeLargeVal,feeFood:feeFoodVal,capacity:parseInt(document.getElementById('m-capacity').value)||30,dates:[...state._tempDates],banner:state._tempBanner||null,published:false,notes:''};if(state.editMarketId){var i=state.markets.findIndex(x=>x.id===state.editMarketId);m.published=state.markets[i].published;m.notes=state.markets[i].notes||'';state.markets[i]=m;}else state.markets.push(m);sbSave('markets',marketToDb(m));closeModal('market-modal');renderMarkets();updateMetrics();}
