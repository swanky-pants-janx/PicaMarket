...
// ── DB HELPERS ────────────────────────────────────────────────────
function dbToMarket(r){return{id:r.id,name:r.name,desc:r.description||'',header:r.header||'',fee:r.fee||FEE,stallTypes:r.stall_types||[],capacity:r.capacity||30,dates:r.dates||[],deadline:r.deadline||null,banner:r.banner||null,published:r.published||false,notes:r.notes||''};}
function marketToDb(m){return{id:m.id,user_id:currentUser.id,name:m.name,description:m.desc||'',header:m.header||'',fee:m.fee||FEE,stall_types:m.stallTypes||[],capacity:m.capacity||30,dates:m.dates||[],deadline:m.deadline||null,banner:m.banner||null,published:!!m.published,notes:m.notes||''};}
...
function toggleAttendance(vid,mid){var v=state.vendors.find(x=>x.id===vid);if(!v)return;if(!v.marketAttendance)v.marketAttendance={};v.marketAttendance[mid]=!v.marketAttendance[mid];sbSave('vendors',vendorToDb(v));openMarketDashboard(mid);}
