...
// ── EMAIL ─────────────────────────────────────────────────────────
function sendEmail(to,subject,html){if(!currentUser||!currentUser.id){console.error('Email error: no user');return;}fetch('https://bjzckhanxudkyrpczqbs.supabase.co/functions/v1/resend-email',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer sb_publishable_f64M7MFa88zOMuZ083v-lw_Ypgcyhx-'},body:JSON.stringify({to,subject,html,user_id:currentUser.id})}).catch(err=>console.error('Email error:',err));}
...
async function savePaymentSettings(){
  ...
  var{error}=await _sb.from('profiles').update({payfast_merchant_id:mid,payfast_merchant_key:mkey,payfast_passphrase:passphrase||null,bank_holder:bankHolder,bank_name:bankName,bank_acc_num:bankAccNum,bank_branch:bankBranch,bank_acc_type:bankAccType}).eq('id',currentUser.id);
  btn.textContent='Save';btn.disabled=false;
  if(error){alert('Failed to save. Try again.');return;}
  currentUser.payfastMerchantId=mid||null;currentUser.payfastMerchantKey=mkey||null;currentUser.payfastPassphrase=passphrase||null;
...
async function deleteMarket(id){var cnt=state.vendors.filter(v=>v.status==='approved'&&v.markets.includes(id)).length;if(cnt>0){alert('This market has '+cnt+' vendor'+(cnt!==1?'s':'')+' attending. Remove all vendors before deleting this market.');return;}await sbDel('markets',id);state.markets=state.markets.filter(x=>x.id!==id);renderMarkets();updateMetrics();}
...
async function saveMarket(){var name=document.getElementById('m-name').value.trim();if(!name){alert('Market name is required.');return;}var stallTypesVal=state._tempStallTypes.filter(t=>t.name&&t.name.trim()).map(t=>({id:t.id||('st-'+Date.now()),name:t.name.trim(),fee:t.fee||FEE,color:t.color||'#6b7280'}));if(!stallTypesVal.length)stallTypesVal=defaultStallTypes(FEE);var m={id:state.editMarketId||uid(),name,desc:document.getElementById('m-desc').value.trim(),header:document.getElementById('m-header').value.trim(),fee:stallTypesVal[0].fee,stallTypes:stallTypesVal,capacity:parseInt(document.getElementById('m-capacity').value)||30,dates:[...state._tempDates],deadline:document.getElementById('m-deadline').value||null,banner:state._tempBanner||null,published:false,notes:''};if(state.editMarketId){var i=state.markets.findIndex(x=>x.id===state.editMarketId);m.published=state.markets[i].published;m.notes=state.markets[i].notes||'';state.markets[i]=m;}else state.markets.push(m);await sbSave('markets',marketToDb(m));showPage('markets');renderMarkets();updateMetrics();}
