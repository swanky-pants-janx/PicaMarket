...
// ── NAVIGATION ────────────────────────────────────────────────────
function showPage(p){document.querySelectorAll('.page').forEach(el=>el.classList.remove('active'));document.querySelectorAll('.nav-tab').forEach(el=>el.classList.remove('active'));document.querySelectorAll('.bottom-nav-item').forEach(el=>el.classList.remove('active'));document.querySelectorAll('.sidebar-item').forEach(el=>el.classList.remove('active'));document.getElementById('page-'+p).classList.add('active');var map={home:0,approval:1,approved:2,markets:3,'form-builder':4};if(map[p]!==undefined){var nt=document.querySelectorAll('.nav-tab')[map[p]];if(nt)nt.classList.add('active');var bn=document.querySelectorAll('.bottom-nav-item')[map[p]];if(bn)bn.classList.add('active');var si=document.querySelectorAll('.sidebar-item')[map[p]];if(si)si.classList.add('active');}if(p==='home')renderHome();if(p==='approval'){renderPending();clearNotifications();}if(p==='approved')renderApproved();if(p==='markets')renderMarkets();if(p==='form-builder')renderFormBuilder();if(p==='public')renderPublic();if(state.hideHints)document.querySelectorAll('.page-banner').forEach(el=>el.style.display='none');updateMetrics();window.scrollTo(0,0);}
