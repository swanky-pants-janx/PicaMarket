...
function startVendorWatch(){if(_realtimeChannel){_sb.removeChannel(_realtimeChannel);_realtimeChannel=null;}_notifCount=0;updateBellBadge(0);_realtimeChannel=_sb.channel('vendor-watch-'+currentUser.id).on('postgres_changes',{event:'INSERT',schema:'public',table:'vendors',filter:'user_id=eq.'+currentUser.id},function(payload){var newVendor=dbToVendor(payload.new);if(!state.vendors.find(function(v){return v.id===newVendor.id;}))state.vendors.push(newVendor);var onApproval=document.getElementById('page-approval').classList.contains('active');if(onApproval){renderPending();updateMetrics();showToast('New application: '+(newVendor.name||'Vendor'));}else{_notifCount++;updateBellBadge(_notifCount);showToast('New vendor application!');}}).subscribe();}
...
async function openAccountSettings(){document.getElementById('acc-name').value=currentUser.name||'';document.getElementById('acc-market').value=currentUser.market||'';document.getElementById('acc-email').value=currentUser.email||'';document.getElementById('account-modal').classList.add('open');closeSettingsMenu();}
...
var _realtimeChannel=null;var _notifCount=0;
...
function deleteMarket(id){var cnt=state.vendors.filter(v=>v.status==='approved'&&v.markets.includes(id)).length;if(cnt>0){alert('This market has '+cnt+' vendor'+(cnt!==1?'s':'')+' attending. Remove all vendors before deleting this market.');return;}sbDel('markets',id);state.markets=state.markets.filter(x=>x.id!==id);renderMarkets();updateMetrics();}
