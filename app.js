...
function startVendorWatch(){if(_realtimeChannel){_sb.removeChannel(_realtimeChannel);_realtimeChannel=null;}_notifCount=0;updateBellBadge(0);_realtimeChannel=_sb.channel('vendor-watch-'+currentUser.id).on('postgres_changes',{event:'INSERT',schema:'public',table:'vendors',filter:'user_id=eq.'+currentUser.id},function(payload){var newVendor=dbToVendor(payload.new);if(!state.vendors.find(function(v){return v.id===newVendor.id;}))state.vendors.push(newVendor);var onApproval=document.getElementById('page-approval').classList.contains('active');if(onApproval){renderPending();updateMetrics();showToast('New application: '+(newVendor.name||'Vendor'));}else{_notifCount++;updateBellBadge(_notifCount);showToast('New vendor application!');}}).on('postgres_changes',{event:'UPDATE',schema:'public',table:'vendors',filter:'user_id=eq.'+currentUser.id},function(payload){var updated=dbToVendor(payload.new);var idx=state.vendors.findIndex(function(v){return v.id===updated.id;});if(idx>=0){var prev=state.vendors[idx];state.vendors[idx]=updated;if(updated.payStatus==='paid'&&prev.payStatus!=='paid'){var onApproved=document.getElementById('page-approved').classList.contains('active');if(onApproved){showToast('Payment received — '+updated.name);}else{_notifCount++;updateBellBadge(_notifCount);showToast('Payment received — '+updated.name);}}}else{state.vendors.push(updated);}updateMetrics();var onApproved2=document.getElementById('page-approved').classList.contains('active');if(onApproved2)renderApproved();}).subscribe();}
