...
var _isDevEnv=true; // Turnstile disabled on playwright-test branch
...
// ── DB HELPERS ────────────────────────────────────────────────────
function dbToMarket(r){return{id:r.id,name:r.name,desc:r.description||'',header:r.header||'',fee:r.fee||FEE,stallTypes:r.stall_types||[],capacity:r.capacity||30,dates:r.dates||[],deadline:r.deadline||null,startTime:r.start_time||null,endTime:r.end_time||null,banner:r.banner||null,published:r.published||false,notes:r.notes||'',todoBoard:r.todo_board||null};}
function marketToDb(m){return{id:m.id,user_id:currentUser.id,name:m.name,description:m.desc||'',header:m.header||'',fee:m.fee||FEE,stall_types:m.stallTypes||[],capacity:m.capacity||30,dates:m.dates||[],deadline:m.deadline||null,start_time:m.startTime||null,end_time:m.endTime||null,banner:m.banner||null,published:!!m.published,notes:m.notes||'',todo_board:m.todoBoard||null};}
...
// ── TODO BOARD ────────────────────────────────────────────────────
var _tbDrag=null;
function _tbGetBoard(mid){var m=state.markets.find(x=>x.id===mid);if(!m)return null;if(!m.todoBoard)m.todoBoard={columns:[]};return m.todoBoard;}
async function _tbSave(mid){var m=state.markets.find(x=>x.id===mid);if(m)await sbSave('markets',marketToDb(m));}
function _tbCardHtml(card,mid,colId){return'<div class="todo-card" draggable="true" id="tcard-'+card.id+'" ondragstart="tbDragStart(event,\''+mid+'\',\''+colId+'\',\''+card.id+'\')" ondragend="tbDragEnd(event)"><label style="display:flex;align-items:flex-start;gap:8px;cursor:pointer;flex:1;min-width:0"><input type="checkbox"'+(card.done?' checked':'')+' onchange="tbToggleCard(\''+mid+'\',\''+colId+'\',\''+card.id+'\')" style="margin-top:2px;accent-color:var(--blue);flex-shrink:0"><span class="todo-card-text'+(card.done?' done':'')+'">'+esc(card.text)+'</span></label><button class="todo-card-del" onclick="tbDeleteCard(\''+mid+'\',\''+colId+'\',\''+card.id+'\')" title="Delete">&times;</button></div>';}
function renderTodoBoard(m){
  var board=m.todoBoard||{columns:[]};
  var mid=m.id;
  var cols=board.columns||[];
  var colsHtml=cols.map(function(col){
    var cards=col.cards||[];
    var doneCount=cards.filter(function(c){return c.done;}).length;
    var countBadge=cards.length?'<span style="font-size:11px;color:var(--text3)">'+doneCount+'/'+cards.length+'</span>':'';
    return'<div class="todo-col" id="tcol-'+col.id+'" ondragover="tbDragOver(event)" ondrop="tbDrop(event,\''+mid+'\',\''+col.id+'\')">'+
      '<div class="todo-col-header">'+
        '<div style="display:flex;align-items:center;gap:6px;min-width:0">'+
          '<span class="todo-col-title" onclick="tbRenameCol(\''+mid+'\',\''+col.id+'\')" title="Click to rename">'+esc(col.title)+'</span>'+
          countBadge+
        '</div>'+
        '<button class="todo-col-del" onclick="tbDeleteColumn(\''+mid+'\',\''+col.id+'\')" title="Delete column">&times;</button>'+
      '</div>'+
      '<div class="todo-cards" id="tcards-'+col.id+'">'+cards.map(function(card){return _tbCardHtml(card,mid,col.id);}).join('')+'</div>'+
      '<div class="todo-add-row">'+
        '<input class="form-input todo-add-input" id="tadd-'+col.id+'" type="text" placeholder="Add a card…" onkeydown="tbInputKey(event,\''+mid+'\',\''+col.id+'\')">'+
        '<button class="btn small" onclick="tbAddCardFromBtn(\''+mid+'\',\''+col.id+'\')">+</button>'+
      '</div>'+
    '</div>';
  }).join('');
  var body=cols.length?'<div class="todo-board">'+colsHtml+'</div>':'<div class="empty-state" style="padding:1.5rem;text-align:center">No columns yet. Click "+ Add column" to get started.</div>';
  return'<div class="card" style="margin-bottom:1rem"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:'+(cols.length?'1rem':'0.75rem')+'"><h3 style="margin:0">Task board</h3><button class="btn small" onclick="tbAddColumn(\''+mid+'\')">+ Add column</button></div>'+body+'</div>';
}
function tbInputKey(event,mid,colId){if(event.key==='Enter')tbAddCardFromBtn(mid,colId);}
function tbAddCardFromBtn(mid,colId){var input=document.getElementById('tadd-'+colId);if(!input)return;var text=input.value.trim();if(!text)return;var board=_tbGetBoard(mid);if(!board)return;var col=board.columns.find(function(c){return c.id===colId;});if(!col)return;if(!col.cards)col.cards=[];var card={id:'card-'+Date.now(),text:text,done:false};col.cards.push(card);input.value='';_tbSave(mid);var cardsEl=document.getElementById('tcards-'+colId);if(cardsEl)cardsEl.insertAdjacentHTML('beforeend',_tbCardHtml(card,mid,colId));}
function tbToggleCard(mid,colId,cardId){var board=_tbGetBoard(mid);if(!board)return;var col=board.columns.find(function(c){return c.id===colId;});if(!col)return;var card=(col.cards||[]).find(function(c){return c.id===cardId;});if(!card)return;card.done=!card.done;_tbSave(mid);var textEl=document.querySelector('#tcard-'+cardId+' .todo-card-text');if(textEl)textEl.classList.toggle('done',card.done);}
function tbDeleteCard(mid,colId,cardId){var board=_tbGetBoard(mid);if(!board)return;var col=board.columns.find(function(c){return c.id===colId;});if(!col)return;col.cards=(col.cards||[]).filter(function(c){return c.id!==cardId;});_tbSave(mid);var el=document.getElementById('tcard-'+cardId);if(el)el.remove();}
function tbAddColumn(mid){var title=prompt('Column name:');if(!title||!title.trim())return;var board=_tbGetBoard(mid);if(!board)return;board.columns.push({id:'col-'+Date.now(),title:title.trim(),cards:[]});_tbSave(mid);openMarketDashboard(mid);}
function tbDeleteColumn(mid,colId){var board=_tbGetBoard(mid);if(!board)return;var col=board.columns.find(function(c){return c.id===colId;});if(!col)return;if(col.cards&&col.cards.length&&!confirm('Delete "'+col.title+'" and its '+col.cards.length+' card(s)?'))return;board.columns=board.columns.filter(function(c){return c.id!==colId;});_tbSave(mid);openMarketDashboard(mid);}
function tbRenameCol(mid,colId){var board=_tbGetBoard(mid);if(!board)return;var col=board.columns.find(function(c){return c.id===colId;});if(!col)return;var newTitle=prompt('Rename column:',col.title);if(!newTitle||!newTitle.trim())return;col.title=newTitle.trim();_tbSave(mid);var titleEl=document.querySelector('#tcol-'+colId+' .todo-col-title');if(titleEl)titleEl.textContent=col.title;}
function tbDragStart(event,mid,colId,cardId){_tbDrag={mid:mid,colId:colId,cardId:cardId};event.dataTransfer.effectAllowed='move';setTimeout(function(){var el=document.getElementById('tcard-'+cardId);if(el)el.classList.add('dragging');},0);}
function tbDragEnd(event){if(event.target)event.target.classList.remove('dragging');}
function tbDragOver(event){event.preventDefault();event.dataTransfer.dropEffect='move';}
function tbDrop(event,mid,targetColId){event.preventDefault();if(!_tbDrag||_tbDrag.mid!==mid){_tbDrag=null;return;}var srcColId=_tbDrag.colId,cardId=_tbDrag.cardId;_tbDrag=null;if(srcColId===targetColId)return;var board=_tbGetBoard(mid);if(!board)return;var srcCol=board.columns.find(function(c){return c.id===srcColId;});var tgtCol=board.columns.find(function(c){return c.id===targetColId;});if(!srcCol||!tgtCol)return;var idx=(srcCol.cards||[]).findIndex(function(c){return c.id===cardId;});if(idx===-1)return;var card=srcCol.cards.splice(idx,1)[0];if(!tgtCol.cards)tgtCol.cards=[];tgtCol.cards.push(card);_tbSave(mid);openMarketDashboard(mid);}
...
async function duplicateMarket(id){var m=state.markets.find(x=>x.id===id);if(!m)return;var copy=Object.assign({},m,{id:uid(),name:m.name+' (copy)',published:false,notes:'',todoBoard:null,stallTypes:(m.stallTypes||[]).map(t=>({...t}))});state.markets.push(copy);await sbSave('markets',marketToDb(copy));renderMarkets();updateMetrics();}
...
async function saveMarket(){var name=document.getElementById('m-name').value.trim();if(!name){alert('Market name is required.');return;}var stallTypesVal=state._tempStallTypes.filter(t=>t.name&&t.name.trim()).map(t=>({id:t.id||('st-'+Date.now()),name:t.name.trim(),fee:t.fee||FEE,color:t.color||'#6b7280'}));if(!stallTypesVal.length)stallTypesVal=defaultStallTypes(FEE);var m={id:state.editMarketId||uid(),name,desc:document.getElementById('m-desc').value.trim(),header:document.getElementById('m-header').value.trim(),fee:stallTypesVal[0].fee,stallTypes:stallTypesVal,capacity:parseInt(document.getElementById('m-capacity').value)||30,dates:[...state._tempDates],deadline:document.getElementById('m-deadline').value||null,startTime:document.getElementById('m-start-time').value||null,endTime:document.getElementById('m-end-time').value||null,banner:state._tempBanner||null,published:false,notes:''};if(state.editMarketId){var i=state.markets.findIndex(x=>x.id===state.editMarketId);m.published=state.markets[i].published;m.notes=state.markets[i].notes||'';m.todoBoard=state.markets[i].todoBoard||null;state.markets[i]=m;}else state.markets.push(m);await sbSave('markets',marketToDb(m));showPage('markets');renderMarkets();updateMetrics();}
